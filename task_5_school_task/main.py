...
def read_csv(file_path: str) -> list[list[str]]:
    ...
    The header row is excluded from the returned data.
    ...
    # Skip the header row and split each remaining line by comma
    rows = []
    for line in lines[1:]:
...
def read_robots(robots_path: str) -> list[list]:
    ...
    for row in read_csv(robots_path):
        robot_id = row[0]
...
def read_destinations(destinations_path: str) -> list[list]:
    ...
    for row in read_csv(destinations_path):
...
def read_packages(packages_path: str) -> list[list]:
    ...
    for row in read_csv(packages_path):
        package_id = row[0]
        weight = float(row[1])
